namespace HospitalSystem.Models
{
    public class CBTree
    {
        ...
        private Patient? InsertTree(Patient patient, Patient? active){
            if(active == null){
                return new Patient(patient.Name,patient.TCno,patient.Priority,patient.Status,patient.IsHospitalized);
            }
            if(long.Parse(active.TCno)>long.Parse(patient.TCno)){
                ...
                active.Height = height(active); //Döndürme işlemi olmazsa da yükseklik güncellenir.
        ...
        public List<Patient> ToList() //Ekrana yazdırmada kullanacağız.
        {
            ...
            TInOrder(root, result);
            return result;
        }
        private void TInOrder(Patient? curr, List<Patient> result)
        {
            if(curr == null)
            {
                return;
            }
            TInOrder(curr.left,result); //Önce sol alt ağacı gezer. Null ise hemen döner. Sonra listeye işleme yapar.
            result.Add(curr); //Solu ekler, alt ağaca ait kökü ekler ve en son da köke ait sağı ekler.
            TInOrder(curr.right,result); //Sağ alt ağaçı gezer. Null ise hemen döner.
        }
    }
}